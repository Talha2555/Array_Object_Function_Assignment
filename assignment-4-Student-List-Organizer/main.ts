...
function completedAssignment() {
  let completedAssignment = students.filter(
    (students) =>
      students.isAssignmentDone === true && students.seniorStatus === true
  );
...
function notCompletedAssignment() {
  let notCompletedAssignment = students.filter(
    (students) =>
      students.isAssignmentDone === false && students.seniorStatus === true
  );
...
function notSeniorStudents() {
  let notSeniorStudents = students.filter(
    (students) =>
      students.seniorStatus === false && students.isAssignmentDone === true
  );
  console.log(notSeniorStudents);
  console.log(
    ` These students are not seniors but have completed thier assignment`
  );
}
notSeniorStudents();
